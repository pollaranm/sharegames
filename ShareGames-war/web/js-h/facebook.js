...
FB.getLoginStatus(function(response) {
    ...
    if (response.status === 'connected') {
            ...
            FB.api('/me','GET',{"fields":"id,name,picture{url}"},function(response) {
                ...
                $(form).append($(action));  $(form).append($(name)); $(form).append($(id)); $(form).append($(email)); $(form).append($(url));
                $(form).submit();
	  	});
...
function fb_login() {
        ...
        if (response.authResponse) {
            ...
            FB.api('/me','GET',{"fields":"id,name,picture{url}"},function(response) {
                ...
                $(form).append($(input));  $(form).append($(input1)); $(form).append($(input2)); $(form).append($(input3)); $(form).append($(url));
                $(form).submit();
